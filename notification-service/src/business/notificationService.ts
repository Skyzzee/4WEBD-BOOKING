...
const logEvent = async (dto: LogEventDto) => {
    ...
    }).catch(err => console.error(`[NOTIFICATION_SERVICE] Logger service unavailable:`, err.message));
};
...
export const processNotification = async (templateName: string, to: string, data: any) => {
    ...
    if (!builder) {
        logEvent({ level: LogSeverity.WARN, message: `Template ${templateName} non reconnu.` });
    ...
    try {
        ...
        logEvent({ level: LogSeverity.ERROR, message: `Erreur lors de l'envoi de l'email à ${to} pour le template ${templateName}: ${error.message}` });
