...
export const buyTicket = async (eventId: string, userId: string) => {
    const eventResponse = await fetch(`${EVENT_SERVICE_URL}/api/events/internal/${eventId}`, {
        headers: { 'internal-api-key': process.env.INTERNAL_API_KEY || '' }
    });
...
export const getTicketById = async (id: string, userId: string, userRole: string) => {
...
export const validateTicket = async (id: string) => {
    ...
    if (decoded.ticketId !== ticket.id || decoded.userId !== ticket.userId || decoded.eventId !== ticket.eventId) {
...
export const cancelTicket = async (id: string, userId: string, userRole: string) => {
    const ticket = await prisma.ticket.findUnique({ 
        where: { id } 
    });
