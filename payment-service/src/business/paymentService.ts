...
const logEvent = async (dto: LogEventDto) => {
    ...
    }).catch(err => console.error(`[PAYMENT_SERVICE] Logger service unavailable:`, err.message));
};

export const createPayment = async (ticketId: string, userId: string, amount: number) => {
    ...
    // Mettre en place l'apppel stripe ici je pense
...
export const getPaymentByTicketId = async (ticketId: string, userId: string, userRole: string) => {
    const payment = await prisma.payment.findUnique({ 
        where: { ticketId } 
    });
...
export const getPaymentsByEventId = async (eventId: string) => {
    const ticketsResponse = await fetch(`${TICKET_SERVICE_URL}/api/tickets/internal/event/${eventId}`, {
        headers: { 'internal-api-key': process.env.INTERNAL_API_KEY || '' }
    });
...
export const refundPayment = async (id: string) => {
    ...
    if (payment.status !== PaymentStatus.SUCCESS) {
        throw new AppError("Seuls les paiements réussis peuvent être remboursés.", 400);
    }
